...
"""
    ...
    async def generate_terraform_example(self) -> dict[str, Any]:
        """
        Generate an example Terraform plan JSON with intentional security issues.

        Returns:
            Dict with 'example' (the Terraform plan JSON) and 'description' (what issues it contains)
        """
        prompt = """Generate a realistic Terraform plan JSON that contains intentional security vulnerabilities for testing a security analysis tool.

The plan should include 3-5 resource changes with these characteristics:
- At least one security group with overly permissive rules (0.0.0.0/0)
- At least one database or storage resource with encryption disabled
- At least one resource with public accessibility enabled
- Mix of create/update actions
...
Return ONLY valid JSON in this exact format (no markdown, no explanation):
{
  ...
  "resource_changes": [
    {
      "address": "aws_<resource_type>.<resource_name>",
      "mode": "managed",
      "type": "aws_<resource_type>",
      "name": "<resource_name>",
      "change": {
        "actions": ["create"],
        "before": null,
        "after": {
          // resource attributes with security issues
        }
      }
    }
  ]
...
Make it realistic but with clear security issues that would be caught by a security analyzer."""

        try:
            if self.provider == "anthropic" and self.anthropic_client:
                response_text = await self._call_anthropic(prompt)
            elif self.provider == "bedrock" and self.client and self.credentials_valid:
                response_text = await self._call_bedrock(prompt)
            else:
                # Return static example in mock mode
                return self._get_static_terraform_example()
            ...
            example = json.loads(cleaned_text)
            return {
                "example": example,
                "description": "AI-generated Terraform plan with security vulnerabilities for testing",
                "generated": True,
            }

        except Exception as e:
            logger.warning(f"Failed to generate Terraform example: {e}, using static example")
            return self._get_static_terraform_example()

    async def generate_iam_example(self) -> dict[str, Any]:
        """
        Generate an example IAM policy JSON with intentional security issues.

        Returns:
            Dict with 'example' (the IAM policy JSON) and 'description' (what issues it contains)
        """
        prompt = """Generate a realistic AWS IAM policy JSON that contains intentional security vulnerabilities for testing a security analysis tool.

The policy should include:
- At least one statement with Action: "*" (wildcard permissions)
- At least one statement with Resource: "*" (all resources)
- At least one overly permissive service action (like s3:*, iam:*, ec2:*)
- Mix of Allow and potentially risky permissions

Return ONLY valid JSON in this exact IAM policy format (no markdown, no explanation):
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
Make it realistic but with clear privilege escalation and security issues that would be caught by an IAM policy analyzer."""
        ...
        try:
            if self.provider == "anthropic" and self.anthropic_client:
                ...
                # Return static example in mock mode
                return self._get_static_iam_example()
            ...
            return {
                "example": example,
                "description": "AI-generated IAM policy with security vulnerabilities for testing",
                "generated": True,
            }

        except Exception as e:
            logger.warning(f"Failed to generate IAM example: {e}, using static example")
            return self._get_static_iam_example()

    def _get_static_terraform_example(self) -> dict[str, Any]:
        """Return a static Terraform plan example for mock mode."""
        example = {
            "format_version": "0.1",
            "resource_changes": [
                {
                    ...
                    "change": {
                        ...
                        "after": {
                            ...
                            "ingress": [{"cidr_blocks": ["0.0.0.0/0"], "from_port": 0, "protocol": "-1", "to_port": 0}],
                        },
                    },
                },
                {
        ...
        return {
            "example": example,
            "description": "Static example Terraform plan with security vulnerabilities",
            "generated": False,
        }

    def _get_static_iam_example(self) -> dict[str, Any]:
        """Return a static IAM policy example for mock mode."""
        example = {
            "Version": "2012-10-17",
            "Statement": [
        ...
        return {
            "example": example,
            "description": "Static example IAM policy with security vulnerabilities",
            "generated": False,
        }
