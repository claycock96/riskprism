...
from app.models import PlanSummary, ResourceChange, RiskFinding
...
class TerraformAnalyzer(BaseAnalyzer):
    ...
    def parse(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        self._resource_hash_map = {change.resource_ref: change.resource_address for change in self._diff_skeleton}
    ...
    def analyze(self, parsed_data: dict[str, Any], max_findings: int = 50) -> list[RiskFinding]:
        ...
        """
        return self.risk_engine.analyze(parsed_data, self._diff_skeleton, max_findings=max_findings)
